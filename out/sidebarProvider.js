...
exports.PetJudgeSidebarProvider = void 0;
const vscode = require("vscode");
class PetJudgeSidebarProvider {
    ...
    triggerPoopAnimation() {
        const config = vscode.workspace.getConfiguration('petJudge');
        const autoOpen = config.get('autoOpenSidebar', true);
        if (this._view) {
            if (autoOpen) {
    ...
    _getHtml(webview) {
        ...
        const config = vscode.workspace.getConfiguration('petJudge');
        const defaultSetting = config.get('defaultCharacter', 'dog');
...
exports.PetJudgeSidebarProvider = PetJudgeSidebarProvider;
function getNonce() {
